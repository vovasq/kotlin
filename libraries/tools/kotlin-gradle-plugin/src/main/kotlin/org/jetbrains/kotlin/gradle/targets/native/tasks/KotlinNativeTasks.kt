...
abstract class AbstractKotlinNativeCompile<T : KotlinCommonToolOptions> : AbstractCompile() {
    ...
    protected open fun buildSourceArgs(): List<String> = emptyList()

    protected fun buildArgs(): List<String> =
...
open class KotlinNativeLink : AbstractKotlinNativeCompile<KotlinCommonToolOptions>() {
    ...
    @InputFile
    val sourceLibrary: Provider<File> = project.provider {
        compilation.compileKotlinTask.outputFile.get()
    }

    override fun getSource(): FileTree = project.files(sourceLibrary.get()).asFileTree
    ...
    override fun buildSourceArgs(): List<String> = mutableListOf<String>().apply {
        val sourceLibraryPath = sourceLibrary.get().absolutePath
        addArg("-l", sourceLibraryPath)
        add("-Xsource-library=$sourceLibraryPath")
    }
