...
// After this pass runs there are only three kinds of IrTypeOperatorCalls left:
//
// - IMPLICIT_CAST
...
//
// The latter two correspond to the instanceof/checkcast instructions on the JVM.
...
private class TypeOperatorLowering(private val context: JvmBackendContext) : FileLoweringPass, IrBuildingTransformer(context) {
    private val unitInstanceField by lazy {
        context.declarationFactory.getFieldForObjectInstance(context.irBuiltIns.unitClass.owner)
    }
        ...
        return when (expression.operator) {
            IrTypeOperator.IMPLICIT_COERCION_TO_UNIT -> {
                irComposite {
                    ...
                    +IrGetFieldImpl(UNDEFINED_OFFSET, UNDEFINED_OFFSET, unitInstanceField.symbol, expression.type)
                }
            }
